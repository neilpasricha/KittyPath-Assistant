...
def jayBoyToJSON(classArray, creditArray, semester):
...
#planUrl = "https://rap.psu.edu/accounting-acct-recommended-academic-plan"
#planUrl = "https://rap.psu.edu/applied-behavioral-science-abesc-recommended-academic-plan"
planUrl = "https://rap.psu.edu/electrical-engineering-technology-e-e-t-bs-general-option-recommended-academic-plan"


'''
with open("PSUHarrisburg_Plan_Links", 'r') as readFile:
...
for i in planSoup.find_all("div", {"class": "rap-field-collection"}):
    ...
    for line in i.find_all("tr", {"class": "field_collection_item"}):
        ...
        '''
        garbageTest = line.find_all("div", {"class": "printfloat"})
        
        for ball in garbageTest:
            #print(ball.text)
            #textList.append(ball.text)
            
            ors = ball.find_all("div", {"class": "showprint"})

            if(len(ors) == 0):
                textList.append(ball.text)
            else:
                for o in ors:
                    textList.append(o.text)
            

        garbageTest = line.find_all("div", {"class": "orcourse"})
        for ball in garbageTest:
            #print("!!!"+ball.text+"!!!!")
            ...
            if(len(ors) == 0):
                if(textList[len(textList)-1] == ball.text):
                    textList[len(textList)-1] = "or " + ball.text
                else:
                    textList.append(ball.text)
            else:
                for o in ors:
    ...
    jayBoyToJSON(semClasses, semCreds, semester)
